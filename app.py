...
# 2. عرض الشروط والجوائز والنقاط المعتمدة
with st.expander("📜 شروط وقوانين المسابقة والجوائز (اضغط للتفاصيل)", expanded=False):
    st.markdown(
        ...
        <hr>
        <h3>2️⃣ نظام القفل التلقائي وحفظ المباريات ⏱️:</h3>
        <ul>
            <li>تقدر تتوقع <b>كل مباراة لحالها وتحفظها بشكل مستقل</b>، مو شرط تتوقع الجولة كلها مرة وحدة!</li>
...
st.info("💡 اكتب اسمك ورمزك السري أولاً لتتمكن من حفظ توقعاتك لكل مباراة بشكل مستقل.")
...
# 4. قائمة مباريات الجولة الأولى الرسمية والمصححة بالأعلام الكاملة وتوقيت مكة
...
matches_round_1 = [
    ...
    {"id": "m16", "date": "الثلاثاء 16/06/2026 - 04:00 ص", "team1": "إران 🇮🇷", "team2": "نيوزيلندا 🇳🇿"},
...
# عرض المباريات مع خيارات نظيفة وأزرار حفظ منفصلة
for match in matches_round_1:
    with st.container():
        st.markdown(f"""
            <div class="match-card">
                <b style='color: #4B5563;'>🗓️ {match['date']}</b><br>
        ...
        choice = st.radio(f"اختر توقعك:", options, key=f"radio_{match['id']}")
        
        if st.button(f"💾 حفظ توقع مباراة {match['team1']} ضد {match['team2']}", key=f"btn_{match['id']}"):
            if user_name and user_code:
                st.success(f"✔️ تم تأمين وتثبيت توقعك لهذه المباراة بنجاح يا {user_name}! ({choice})")
            else:
                st.error("⚠️ خطأ: يرجى كتابة اسمك والرمز السري في أعلى الصفحة أولاً لحفظ التوقع!")
        st.write("---")
...
st.write("تقدر تعبيها الحين وتحفظها منفصلة، وبتفتح النفس لنهاية البطولة!")
...
if st.button("💾 حفظ التوقعات الذهبية الكبرى"):
    if user_name and user_code:
        st.success(f"🔥 كفو! تم تثبيت توقعاتك الذهبية للأبطال بنجاح يا {user_name}!")
        st.balloons()
    else:
